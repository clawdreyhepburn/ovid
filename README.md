<p align="center">
  ...
  <p align="center">
    Cryptographically signed, short-lived credentials that tell you exactly who a sub-agent is, what role it plays, and who created it.
  </p>
  <p align="center">
...
OVID gives every sub-agent its own identity document — a signed JWT that says who it is, what role it plays, who created it, and when it expires. The spawning agent signs it. The chain is verifiable back to the human.

OVID handles **identity**. What the agent is *authorized to do* is a separate concern — that's the job of a policy engine like [Cedar/Carapace](https://github.com/clawdreyhepburn/carapace).
...
// Spawn a sub-agent with a signed identity
...
if (result.valid) {
  console.log(result.principal);  // "clawdrey/code-reviewer-7f3a"
...
console.log(helper.claims.parent_chain); // ["clawdrey/code-reviewer-7f3a"]
```

The child's lifetime can never exceed the parent's remaining lifetime. The library enforces this at issuance time.
...
| `issuerOvid` | `OvidToken` | no | — | Parent's OVID (omit for root agent) |
...
### `verifyOvid(jwt: string, issuerPublicKey: CryptoKey, options?: VerifyOvidOptions): Promise<OvidResult>`

Verifies an OVID JWT's signature and claims.

| Option | Type | Required | Description |
|--------|------|----------|-------------|
| `trustedRoots` | `CryptoKey[]` | no | Public keys of trusted root agents |
| `maxChainDepth` | `number` | no | Maximum allowed chain depth |

Returns `OvidResult` with `valid`, `principal`, `role`, `chain`, `expiresIn`.
...
```json
{
  "jti": "clawdrey/code-reviewer-7f3a",
  "iss": "clawdrey",
  "sub": "clawdrey/code-reviewer-7f3a",
  "iat": 1711987200,
  "exp": 1711989000,
  "ovid_version": 1,
  "role": "code-reviewer",
...
OVID is the identity layer. [Carapace](https://github.com/clawdreyhepburn/carapace) is the authorization layer. Together:
...
4. Cedar policies evaluate against role, parent chain, depth, and attestation status
...
**Without Carapace**, OVID still works as a standalone identity library. Verify OVIDs in your own code and make authorization decisions however you like.

---

## Identity vs Authorization

OVID deliberately separates identity from authorization:

| Concern | OVID | Carapace/Cedar |
|---------|------|----------------|
| Who is this agent? | ✅ | — |
| Who created it? | ✅ | — |
| When does it expire? | ✅ | — |
| What role does it claim? | ✅ | — |
| What can it actually do? | — | ✅ |
| Is this request permitted? | — | ✅ |
| Is the child's authority provably ≤ parent's? | — | ✅ |

The role claim is for **human readability and audit trails**. Authorization decisions come from Cedar policies that reference role, parent chain, and any other attributes. The policy engine — not the credential — decides what's allowed.
...
- **Not access control.** OVID provides identity. Access control decisions belong to a policy engine like Cedar/Carapace, OpenClaw's native tool policy, or your own code.
...
Allowlists tell the runtime what to restrict, but they don't give the sub-agent a verifiable credential. The sub-agent can't prove its identity to downstream systems. There's no audit trail. See [the blog post](https://clawdrey.com/blog/your-sub-agents-are-running-with-scissors.html).
...
No. OVID is a standalone identity library. You can verify OVIDs and make authorization decisions in your own code. Carapace adds Cedar-based policy enforcement and formal attenuation proofs, but it's optional.
