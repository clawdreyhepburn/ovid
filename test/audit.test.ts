...
describe('AuditLogger', () => {
  ...
  it('logs issuance when creating an OVID', async () => {
    ...
    await createOvid({ issuerKeys: keys, role: 'worker', auditLogger: logger });
  ...
  it('logs verification when verifying an OVID', async () => {
    const logger = createAuditLogger(TEST_LOG);
    const keys = await generateKeypair();
    const token = await createOvid({ issuerKeys: keys, role: 'reader' });
  ...
  it('logs decisions', () => {
    const logger = createAuditLogger(TEST_LOG);
    logger.logDecision('agent-1', 'use_tool', 'browser', 'allow-proven', ['policy-1']);

    const lines = readLines();
    expect(lines).toHaveLength(1);
    const entry = lines[0] as Record<string, unknown>;
    expect(entry.event).toBe('decision');
    expect(entry.decision).toBe('allow-proven');
